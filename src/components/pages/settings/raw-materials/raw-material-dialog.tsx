...
interface RawMaterialDialogProps {
  ...
  handleSubmit: UseFormHandleSubmit<RawMaterialFormData>;
  onSubmit: (data: RawMaterialFormData) => Promise<void>;
  errors: FieldErrors<RawMaterialFormData>;
  loading: boolean;
  error: string;
  ...
  return (
    <Dialog open={open} onOpenChange={() => onClose()}>
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>
            {isEditing ? t("editRawMaterial") : t("addRawMaterial")}
          </DialogTitle>
        </DialogHeader>

        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div className="space-y-4">
            {unitsLoading || categoriesLoading ? (
              <div className="text-center py-4 text-gray-500">
                {t("loadingUnits")}
              </div>
            ) : (
              getRawMaterialFormConfig(t, units, categories).map((field) => (
                <Controller
                  key={field.name}
                  name={field.name}
                  control={control}
                  ...
                  render={({ field: { onChange, onBlur, value, ref } }) => (
                    <Input
                      ...
                      onBlur={onBlur}
                      disabled={field.disabled || loading || unitsLoading || categoriesLoading}
          ...
          <DialogFooter className="mt-4">
            ...
            <Button
              type="submit"
              disabled={loading || unitsLoading || categoriesLoading}
