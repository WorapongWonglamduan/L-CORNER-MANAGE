  ...
  const columns: DataTableColumn<Product>[] = [
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-xs text-gray-500 dark:text-gray-400">
              {product.product_type.name_i18n[locale as "th" | "en"]}
            </div>
          </div>
        </div>
      ),
    },
    {
      ...
      cell: (product) => {
        ...
        const percentage =
          (Number(product.available_quantity) / Number(product.low_stock_threshold)) * 100;
        return (
          <>
            <div className="flex items-center gap-2">
              <div className="text-sm font-semibold text-gray-900 dark:text-white">
                {Number(product.available_quantity).toLocaleString()}{" "}
                {product.base_unit.abbreviation_i18n[locale as "th" | "en"]}
              </div>
              {isLowStock && <TrendingDown className={`w-4 h-4 ${status.textColor}`} />}
            </div>
            {isLowStock && (
              <div className="mt-1 bg-gray-200 rounded-full h-1.5 overflow-hidden w-24">
                <div
                  className={`h-full ${status.color === "red" ? "bg-red-500" : "bg-orange-500"}`}
      ...
      cell: (product) => (
        <div className="flex items-center gap-2">
          <Button
            ...
            title={!filters.warehouseId ? t("selectBranchFirst") : undefined}
          >
            {t("adjustStock")}
          </Button>
          <Button
            ...
            <>
              <DataTable
                ...
                rowClassName={(product) => {
                  const status = getStockStatus(product);
                  const isLowStock = status.label !== "normal";
                  return isLowStock ? `${status.bgColor} border-l-4 ${status.borderColor}` : "";
                }}
              />
